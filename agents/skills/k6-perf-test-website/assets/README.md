...
Scaffolded by the `perf-test-website` skill. End-to-end k6 perf
...
├── recordings/
│   ├── scripts/              # Playwright recorder per workflow (wN.js)
...
node recordings/scripts/w1.js
npx har-to-k6 recordings/har/w1.har -o tests/w1-<name>/from-har.js
...
./tools/run-with-monitor.sh tests/w1-<name>/smoke.js
./tools/run-with-monitor.sh tests/w1-<name>/average.js
./tools/run-with-monitor.sh tests/w1-<name>/stress.js
# ... etc per the run plan in runbook.md

# 5. (If cloud) push to Grafana Cloud k6
k6 cloud run tests/w1-<name>/stress.js
```
...
cp recordings/scripts/recorder.template.js recordings/scripts/wN.js

# Replace <WORKFLOW_PLACEHOLDER> markers in each copied file
grep -rl '<WORKFLOW_PLACEHOLDER>' tests/wN-<short-name> recordings/scripts/wN.js \
...
$EDITOR recordings/scripts/wN.js
node recordings/scripts/wN.js
npx har-to-k6 recordings/har/wN.har -o tests/wN-<short-name>/from-har.js
