...
(`recordings/scripts/wN.js`) that captures a HAR. The HAR is then
...
```
recordings/scripts/wN.js          (Playwright -- source of truth)
├── node recordings/scripts/wN.js  ⟶  recordings/har/wN.har
│      └── npx har-to-k6 wN.har    ⟶  tests/wN-…/from-har.js  (raw)
│            └── hand cleanup       ⟶  tests/wN-…/protocol.js  (final)
└── 5-step manual conversion        ⟶  tests/wN-…/browser.js   (final)
```
...
After `node recordings/scripts/wN.js`:

```bash
# Count entries by host
jq '.log.entries | map(.request.url) | group_by(. | capture("//(?<h>[^/:]+)").h) | map({host: .[0] | capture("//(?<h>[^/:]+)").h, count: length})' recordings/har/wN.har
```
...
```bash
k6 run --vus 1 --iterations 1 tests/wN-<name>/from-har.js
```
