...
- [`protocol.js` — cleaning `from-har.js`](#protocoljs-cleaning-from-harjs)
  ...
  - Step 4: Replace recorded credentials with test credentials
    - [Cookies, CSRF, and k6's CookieJar API](#cookies-csrf-and-k6s-cookiejar-api) — `r.cookies` vs `r.json()`; `cookieJar:` vs `cookies:` footgun
...
VU), the option name is `cookieJar:`, not `cookies:`:

```js
// WRONG — k6 silently ignores this. Your jar stays empty.
const jar = http.cookieJar();
http.get(url, { cookies: jar });

// RIGHT — use cookieJar:, or just rely on the default per-VU jar.
const jar = new http.CookieJar();
http.get(url, { cookieJar: jar });
```
...
# Just one:
k6 run tests/w1-<name>/protocol.js
K6_BROWSER_HEADLESS=true k6 run tests/w1-<name>/browser.js
```
